...
Public Class clsScreenCapture
    ...
    Private PicForm As New PictureBox
    ...
    '/ Captures a screen shot of a specific window, and saves it to a file
    Public Sub CaptureWindowToPrinter(ByVal mfForm As Form, ByVal format As ImageFormat)
        ' Dim img As Image = CaptureScreen()

        Dim img As Image = CaptureWindow(mfForm.Handle)
        Try
            PicForm.Image = img
            If MsgBox("Press Yes to Print or No to Copy to your Clipboard ('No' will allow pasting into an email or Word document etc).", MsgBoxStyle.YesNo, "Print or Copy to Clipboard?") = MsgBoxResult.Yes Then
                Pd.Print()
            Else
                Clipboard.SetDataObject(img, True)
            End If
        Catch ex As Exception
            Dim a As String = "vfdgsfg"
        End Try
    End Sub 'CaptureWindowToPrinter
    ...
    Public Sub CaptureScreenToFile(ByVal filename As String, ByVal format As ImageFormat)
        Dim img As Image = CaptureScreen() ' CaptureScreen()
        img.Save(filename, format)
